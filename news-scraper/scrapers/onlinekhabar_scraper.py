    ...
    for category, link in urls.items():
        ...
        for div in span_divs:
            ...
            if a_tag:  
                ...
                original_content = "\n".join([para.get_text(strip=True).split('।', 1)[1].strip() if '।' in para.get_text(strip=True) else para.get_text(strip=True) for para in article_soup.find('div', class_='ok18-single-post-content-wrap').find_all('p')[1:]]) if article_soup.find('div', class_='ok18-single-post-content-wrap') else print(f"Original content not found for {article_link}")
